...
#define readBytes(devAddr, reg, len, buffer)	MPU6050_read_bytes(reg, buffer, len)
#define readByte(devAddr, reg, buffer)	MPU6050_read_byte(reg, buffer)
#define readBit(devAddr, reg, bit, dat)	MPU6050_read_bit(reg, bit, dat)
#define readBits(devAddr, reg, bitStart, bitLength, dat)	\
	MPU6050_read_bits(reg, bitStart, bitLength, dat)

#define writeBytes(devAddr, reg, len, buffer)	MPU6050_write_bytes(reg, len, buffer)
#define writeByte(devAddr, reg, buffer)	MPU6050_write_byte(reg, &buffer)
#define writeBit(devAddr, reg, bit, dat)	MPU6050_write_bit(reg, bit, dat)
#define writeBits(devAddr, reg, bitStart, bitLength, dat)	\
	MPU6050_write_bits(reg, bitStart, bitLength, dat)

#define writeWord(devAddr, reg, data)			\
{											\
	stm_printf("Not implement function\n");		\
}

static uint8_t program_verify_buffer[MPU6050_DMP_MEMORY_CHUNK_SIZE];
static uint8_t program_memory_buffer[MPU6050_DMP_MEMORY_CHUNK_SIZE];
static uint8_t buffer[16];

uint32_t MPU6050_read_byte(uint8_t reg, uint8_t *dat)
{
	uint32_t ret;
	ret = iic_read_bytes(MPU6050_DEFAULT_ADDRESS<<1, reg, dat, 1);
	if(!ret){
		stm_printf("Read byte fail, reg: %x\n", reg);
	}
	return ret;
}
...
{
	uint32_t ret;
	ret = iic_read_bytes(MPU6050_DEFAULT_ADDRESS<<1, reg, dat, len);
	if(!ret){
		stm_printf("Read bytes fail, reg: %x\n", reg);
	}
	return ret;
}
...
{
	uint32_t ret;
	ret = iic_write_bytes(MPU6050_DEFAULT_ADDRESS<<1, reg, dat, 1);
	if(ret == FALSE){
		stm_printf("IIC: Write reg: %x, dat: %x fail\n", reg, dat);
	}
	return ret;
}
...
{
	uint32_t ret;
	ret = iic_write_bytes(MPU6050_DEFAULT_ADDRESS<<1, reg, dat, len);
	if(ret == FALSE){
		stm_printf("IIC: Write reg: %x, %d bytes fail\n", reg, len);
	}
	return ret;
}
 ...
 * @param bit_length Number of bits to write (not more than 8)
 * @param dat Right-aligned value to write
	...
	uint8_t bit_length, uint8_t dat)
{
	...
	ret = MPU6050_read_bytes(reg, &readout_val, 1);
	if(!ret){
		return ret;
	}
	mask = ((1 << bit_length) - 1) << (bit_start - bit_length + 1);
	dat <<= (bit_start - bit_length + 1); // shift data into correct position
	dat &= mask; // zero all non-important bits in data
	readout_val &= ~(mask); // zero all important bits in existing byte
	readout_val |= dat; // combine data with existing byte
	
	ret = MPU6050_write_bytes(reg, &readout_val, 1);
	return ret;
}
...
uint32_t MPU6050_write_bit(uint8_t regAddr, uint8_t bitNum, uint8_t data) 
{
	uint8_t b;
	uint32_t ret;
	ret = MPU6050_read_bytes(regAddr, &b, 1);
	if(!ret){
		return ret;
	}
	b = (data != 0) ? (b | (1 << bitNum)) : (b & ~(1 << bitNum));
	return MPU6050_write_bytes(regAddr, &b, 1);
}

/** Read multiple bits in an 8-bit device register.
 * @param reg Register regAddr to read from
 * @param bit_start First bit position to read (0-7)
 * @param bit_length Number of bits to read (not more than 8)
 * @param dat Right-aligned value to read
 * @return Status of operation (TRUE = success)
 */
	...
	uint8_t bit_length, uint8_t *dat)
{
	...
	mask = ((1 << bit_length) - 1) << (bit_start - bit_length + 1);
	readout_val &= (mask); // zero all important bits in existing byte
	readout_val >>= (bit_start - bit_length + 1);
	*dat = readout_val;
	
	return ret;
}

/** read a single bit in an 8-bit device register.
 * @param regAddr Register regAddr to read from
 * @param bitNum Bit position to read (0-7)
 * @param value New bit value to read
 * @return Status of operation (TRUE = success)
 */
...
uint32_t MPU6050_initialize(void)
{
	...
	ret = MPU6050_read_bytes(MPU6050_RA_WHO_AM_I, &device_id, 0x1);
	if(ret == FALSE){
		goto INIT_OVER;
	}
	if(device_id != MPU6050_DEFAULT_ADDRESS){ 
		/* Device "who am I" reg return error */
		ret = FALSE;
		goto INIT_OVER;
	}
	...
	ret = MPU6050_write_bits(MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, MPU6050_CLOCK_PLL_XGYRO);
	if(ret == FALSE){
		return ret;
	}
	ret = MPU6050_write_bits(MPU6050_RA_GYRO_CONFIG, MPU6050_GCONFIG_FS_SEL_BIT, MPU6050_GCONFIG_FS_SEL_LENGTH, MPU6050_GYRO_FS_250);
	if(ret == FALSE){
		return ret;
	}
	ret = MPU6050_write_bits(MPU6050_RA_ACCEL_CONFIG, MPU6050_ACONFIG_AFS_SEL_BIT, MPU6050_ACONFIG_AFS_SEL_LENGTH, MPU6050_ACCEL_FS_2);
	if(ret == FALSE){
		return ret;
	}

	ret = MPU6050_write_bit(MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_SLEEP_BIT, FALSE);
	if(ret == FALSE){
		return ret;
	}
	ret = MPU6050_write_bits(MPU6050_RA_PWR_MGMT_2, 7, 8, 0x0);
	if(ret == FALSE){
		return ret;
	}	
 ...
 */
uint32_t getMultiMasterEnabled() {
 ...
 * @see MPU6050_RA_I2C_MST_CTRL
 */
uint32_t getWaitForExternalSensorEnabled() {
 ...
 * @see MPU6050_RA_I2C_MST_CTRL
 */
uint32_t getSlaveReadWriteTransitionEnabled() {
 ...
 * @see MPU6050_RA_I2C_SLV4_CTRL
 */
uint32_t getSlave4InterruptEnabled() {
 ...
 */
uint32_t getInterruptLatchClear() {
 ...
 */
uint32_t getFSyncInterruptLevel() {
 ...
 */
uint32_t getFSyncInterruptEnabled() {
 ...
 */
uint32_t getClockOutputEnabled() {
 ...
 * @see MPU6050_INTERRUPT_FF_BIT
 **/
uint32_t getIntFreefallEnabled() {
 ...
 **/
uint32_t getIntZeroMotionEnabled() {
 ...
 **/
uint32_t getIntFIFOBufferOverflowEnabled() {
 ...
 **/
uint32_t getIntI2CMasterEnabled() {
 ...
 */
uint32_t getIntDataReadyEnabled() {
 ...
 * @see MPU6050_INTERRUPT_ZMOT_BIT
 */
uint32_t getIntZeroMotionStatus() {
 ...
 * @see MPU6050_INTERRUPT_FIFO_OFLOW_BIT
 */
uint32_t getIntFIFOBufferOverflowStatus() {
 ...
 * @see MPU6050_INTERRUPT_I2C_MST_INT_BIT
 */
uint32_t getIntI2CMasterStatus() {
 ...
 * @see MPU6050_INTERRUPT_DATA_RDY_BIT
 */
uint32_t getIntDataReadyStatus() {
 ...
 */
uint32_t getXNegMotionDetected() {
 ...
 */
uint32_t getXPosMotionDetected() {
 ...
 */
uint32_t getYNegMotionDetected() {
 ...
 */
uint32_t getYPosMotionDetected() {
 ...
 */
uint32_t getZNegMotionDetected() {
 ...
 */
uint32_t getZPosMotionDetected() {
 ...
 */
uint32_t getZeroMotionDetected() {
 ...
 */
uint32_t getExternalShadowDelayEnabled() {
 ...
 */
uint32_t getI2CMasterModeEnabled() {
...
uint32_t reset(void)
{
	...
	/* Reset the device, and wait for ready */
	dat = 0x80;
	ret = MPU6050_write_bit(MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_DEVICE_RESET_BIT, TRUE);
	if(ret == FALSE){
		return FALSE;
	}
	...
	count = 0;
	while(1){
		ret = MPU6050_read_bit(MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_DEVICE_RESET_BIT, &dat);
		if(ret == FALSE){
			break;
		}
		if(dat == FALSE){
			stm_printf("Info: Device reset in %d ms\n", count*2);
			/* Device reset ok */
			break;
		}
		count++;
		...
		if(count > 400){
			stm_printf("Error: Device reset timeout %d ms\n", count*2);
			ret = FALSE;
			break;
		}
	}
 ...
 */
uint32_t getStandbyXAccelEnabled() {
 ...
 */
uint32_t getStandbyYAccelEnabled() {
 ...
 */
uint32_t getStandbyZAccelEnabled() {
 ...
 */
uint32_t getStandbyXGyroEnabled() {
 ...
 */
uint32_t getStandbyYGyroEnabled() {
 ...
 */
uint32_t getStandbyZGyroEnabled() {
...
uint32_t getIntPLLReadyEnabled() {
...
{
	MPU6050_read_bit(MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DMP_INT_BIT, buffer);
	return buffer[0];
}

void setIntDMPEnabled(uint32_t enabled) 
{
	MPU6050_write_bit(MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DMP_INT_BIT, enabled);
}

// DMP_INT_STATUS
uint32_t getDMPInt5Status(void)
{
	MPU6050_read_bit(MPU6050_RA_DMP_INT_STATUS, MPU6050_DMPINT_5_BIT, buffer);
	return buffer[0];
}
uint32_t getDMPInt4Status(void) 
{
   	MPU6050_read_bit(MPU6050_RA_DMP_INT_STATUS, MPU6050_DMPINT_4_BIT, buffer);
	return buffer[0];
}
uint32_t getDMPInt3Status(void) 
{
	MPU6050_read_bit(MPU6050_RA_DMP_INT_STATUS, MPU6050_DMPINT_3_BIT, buffer);
	return buffer[0];
}
uint32_t getDMPInt2Status(void) 
{
	MPU6050_read_bit(MPU6050_RA_DMP_INT_STATUS, MPU6050_DMPINT_2_BIT, buffer);
	return buffer[0];
}
uint32_t getDMPInt1Status(void) 
{
	MPU6050_read_bit(MPU6050_RA_DMP_INT_STATUS, MPU6050_DMPINT_1_BIT, buffer);
	return buffer[0];
}
uint32_t getDMPInt0Status(void) 
{
	MPU6050_read_bit(MPU6050_RA_DMP_INT_STATUS, MPU6050_DMPINT_0_BIT, buffer);
	return buffer[0];
}
...
{
	MPU6050_read_bit(MPU6050_RA_INT_STATUS, MPU6050_INTERRUPT_PLL_RDY_INT_BIT, buffer);
	return buffer[0];
}
uint32_t getIntDMPStatus(void) 
{
	MPU6050_read_bit(MPU6050_RA_INT_STATUS, MPU6050_INTERRUPT_DMP_INT_BIT, buffer);
	return buffer[0];
}
...
{
	MPU6050_read_bit(MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_DMP_EN_BIT, buffer);
	return buffer[0];
}
void setDMPEnabled(uint32_t enabled)
{
	MPU6050_write_bit(MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_DMP_EN_BIT, enabled);
}
void resetDMP(void) 
{
	MPU6050_write_bit(MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_DMP_RESET_BIT, TRUE);
}
...
uint32_t setMemoryBank(uint8_t bank, uint32_t prefetchEnabled, uint32_t userBank) 
{
	...
	ret = MPU6050_write_byte(MPU6050_RA_BANK_SEL, &bank);
	return ret;
}
...
{
	uint32_t ret;
	ret = MPU6050_write_byte(MPU6050_RA_MEM_START_ADDR, &address);
	return ret;
}
...
{
	uint8_t dat;
	MPU6050_read_byte(MPU6050_RA_MEM_R_W, &dat);
	return dat;
}
...
{
	uint32_t ret;
	ret = MPU6050_write_byte(MPU6050_RA_MEM_R_W, &data);
	return ret;
}
...
void readMemoryBlock(uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address) 
{
	...
	for (i = 0; i < dataSize;) {
		...
		MPU6050_read_bytes(MPU6050_RA_MEM_R_W, data + i, chunkSize);
...
uint32_t writeMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address, uint32_t verify, uint32_t useProgMem) 
{
	...
	uint8_t j;
	if (verify) 
		verifyBuffer = program_verify_buffer;
	if (useProgMem) 
		progBuffer = program_memory_buffer;
	...
	for (i = 0; i < dataSize;) {
		...
		if (useProgMem) {
			...
			for (j = 0; j < chunkSize; j++) 
				progBuffer[j] = (*(const unsigned char *)(data + i + j));
		} else {
			// write the chunk of data as specified
			progBuffer = (uint8_t *)data + i;
		}

		MPU6050_write_bytes(MPU6050_RA_MEM_R_W, progBuffer, chunkSize);
...
ReTryRead:
			setMemoryBank(bank, FALSE, FALSE);
			setMemoryStartAddress(address);
			MPU6050_read_bytes(MPU6050_RA_MEM_R_W, verifyBuffer, chunkSize);
			if (MemCmp((const char *)progBuffer, (const char *)verifyBuffer, chunkSize) != 0) {
				// Do not return, only print a warning
				stm_printf("Dat compare fail at %x\n", i);
				setMemoryBank(bank, FALSE, FALSE);
				setMemoryStartAddress(address);
				MPU6050_write_bytes(MPU6050_RA_MEM_R_W, progBuffer, chunkSize);
				goto ReTryRead;
...
uint32_t writeDMPConfigurationSet(const uint8_t *data, uint16_t dataSize, uint32_t useProgMem) 
{
	...
	if (useProgMem) {
		progBuffer = program_memory_buffer; // assume 8-byte blocks, realloc later if necessary
	}

	for (i = 0; i < dataSize;) {
		if (useProgMem) {
			bank =  (*(const unsigned char *)(data + i++));//pgm_read_byte(data + i++);
			offset = (*(const unsigned char *)(data + i++));//pgm_read_byte(data + i++);
			length = (*(const unsigned char *)(data + i++));//pgm_read_byte(data + i++);
		} else {
			bank = data[i++];
			offset = data[i++];
			length = data[i++];
		...
		if (length > 0) {
			if (useProgMem) {
				for (j = 0; j < length; j++) 
					progBuffer[j] =  (*(const unsigned char *)(data + i + j));//pgm_read_byte(data + i + j);
			} else {
				progBuffer = (uint8_t *)data + i;
			}
			...
			if (useProgMem) {
				special =  (*(const unsigned char *)(data + i + j));//pgm_read_byte(data + i++);
			} else {
				special = data[i++];
			}
			if (special == 0x01) {
				...
				uint8_t tmp = 0x32;
				MPU6050_write_bytes(MPU6050_RA_INT_ENABLE, &tmp, 1);
				success = TRUE;
			} else {
...
{
	MPU6050_read_byte(MPU6050_RA_DMP_CFG_1, buffer);
	return buffer[0];
}

void setDMPConfig1(uint8_t config) 
{
	MPU6050_write_byte(MPU6050_RA_DMP_CFG_1, &config);
}
...
{
	uint8_t dat;
	MPU6050_read_byte(MPU6050_RA_DMP_CFG_2, &dat);
	return dat;
}

void setDMPConfig2(uint8_t config) 
{
	MPU6050_write_byte(MPU6050_RA_DMP_CFG_2, &config);
}
